...
file_loc = os.getcwd()
data_loc = os.path.dirname(os.path.dirname(os.getcwd())) + '/Neural_PDE/Data'
...
# %%
#Extracting the Numerical Parameters from the simulation. 
...
def PI_loss(pred, yy):
    return residual(pred).pow(2).mean()

def PISL(pred, yy):
    return (residual(pred) - residual(yy)).pow(2).mean()

def combined(pred, yy):
    pisl = PISL(pred, yy)
    lp = lploss(pred, yy)
    return lp + 1000*pisl
...
if configuration['Loss Function'] == 'Lp':
    loss_func = LpLoss(size_average=False)
elif configuration['Loss Function'] == 'PI':
    loss_func = PI_loss
elif configuration['Loss Function'] == 'PISL':
    loss_func= PISL
elif configuration['Loss Function'] == 'Combined':
...
def train_one_epoch_AR():
    ...
    for xx, yy in train_loader:
        ...
        #PISL Loss 
        # loss = (residual_loss(pred) - residual_loss(yy)).pow(2).mean()
    ...
    with torch.no_grad():
        for xx, yy in test_loader:
            ...
            test_loss += F.mse_loss(pred.reshape(batch_size, -1), yy.reshape(batch_size, -1)).item()
