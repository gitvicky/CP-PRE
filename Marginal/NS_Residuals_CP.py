...
ncf_scores = np.abs(w_cal_bc_residual.numpy())
...
for alpha in tqdm(alpha_levels):
    qhat = calibrate(scores=ncf_scores, n=len(ncf_scores), alpha=alpha)
    prediction_sets = [- qhat, + qhat]
    emp_cov_res.append(emp_cov(prediction_sets, w_pred_bc_residual.numpy()))
...
plt.savefig(os.path.dirname(os.getcwd()) + "/Plots/marginal_NS_BC.svg", format="svg", bbox_inches='tight')
plt.savefig(os.path.dirname(os.getcwd()) + "/Plots/marginal_NS_BC.pdf", format="pdf", bbox_inches='tight')
plt.show()
# %%
