...
"""
# %%
configuration = {"Case": 'Constrained MHD',
...
# %%
data = data_loc + '/Constrained_MHD_combined100.npz'
# %%
field = configuration['Field']
dims = ['rho', 'u', 'v', 'p', 'Bx', 'By']
num_vars = configuration['Variables']

rho_sol = np.load(data)['rho'].astype(np.float32) 
u_sol = np.load(data)['u'].astype(np.float32) 
v_sol = np.load(data)['v'].astype(np.float32)
p_sol = np.load(data)['p'].astype(np.float32)
Bx_sol = np.load(data)['Bx'].astype(np.float32)
By_sol = np.load(data)['By'].astype(np.float32)

rho_sol = np.nan_to_num(rho_sol)
u_sol = np.nan_to_num(u_sol)
v_sol = np.nan_to_num(v_sol)
p_sol = np.nan_to_num(p_sol)
Bx_sol = np.nan_to_num(Bx_sol)
By_sol = np.nan_to_num(By_sol)

rho = torch.from_numpy(rho_sol)
rho = rho.permute(0, 2, 3, 1)
...
Bx = torch.from_numpy(Bx_sol)
Bx = Bx.permute(0, 2, 3, 1)

By = torch.from_numpy(By_sol)
By = By.permute(0, 2, 3, 1)

t_res = configuration['Temporal Resolution']
x_res = configuration['Spatial Resolution']
uvp = torch.stack((rho, u, v, p, Bx, By), dim=1)[:, ::t_res]


x_grid = np.load(data)['x'].astype(np.float32) 
y_grid =x_grid
dt = np.load(data)['dt'].astype(np.float32) 
...
model.load_state_dict(torch.load(model_loc + '/FNO_ConsMHD_quick-rout.pth', map_location='cpu'))
model.to(device)
model.eval()
...
# #Taking upto the 400 time instances
# rho, u, v, p, Bx, By = rho[:t_len], u[:t_len], v[:t_len], p[:t_len], Bx[:t_len], By[:t_len]

# #Slicing to the same spatial and temporal resolution over which the model was trained on. 
# rho = rho[::t_slice, ::x_slice, ::x_slice]
# u = u[::t_slice, ::x_slice, ::x_slice]
# v = v[::t_slice, ::x_slice, ::x_slice]
# p = p[::t_slice, ::x_slice, ::x_slice]
# Bx = Bx[::t_slice, ::x_slice, ::x_slice]
# By = By[::t_slice, ::x_slice, ::x_slice]

# dt=dt*t_slice
# # %% 
# rho_sol = torch.tensor(rho, dtype=torch.float32).unsqueeze(0).permute(0, 2, 3, 1)
...
# Bx_sol = torch.tensor(Bx, dtype=torch.float32).unsqueeze(0).permute(0, 2, 3, 1)
# By_sol = torch.tensor(By, dtype=torch.float32).unsqueeze(0).permute(0, 2, 3, 1)

# uvp = torch.stack((rho_sol, u_sol, v_sol, p_sol, Bx_sol, By_sol), dim=1)[:, ::t_res]
...
for ii in range(num_vars):
    ...
    ax.title.set_text('Actual  - ' + dims[ii])
    ax.set_xlabel('x')
    ax.set_ylabel('y')
