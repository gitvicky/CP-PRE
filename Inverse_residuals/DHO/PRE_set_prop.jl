...
function set_PRE(neural_test)
    ...
    D_pos_kernel = 2*m*D_tt_kernel + dt*c*D_t_kernel + 2*dt^2*k*D_identity
    ...
    kernel_fft = conj.(kernel_fft)

    convolved = ifft(signal_fft .* kernel_fft)
    inverse_kernel = compute_inverse(kernel_fft)

    convolved_noedges = convolved[2:end-3]
    right_edges = convolved[1]
    left_edges = convolved[end-2:end]
...
ID = rand(1:size(numerical_sol, 1))
