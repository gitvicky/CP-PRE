...
for ii in tqdm(range(n_sims)):
    a, b, c = run_sim(ii, params[ii,0], params[ii,1], params[ii,2], params[ii,3]) # a = u, b = D, c = D_x 
    u_dataset.append(a)
    D.append(b)
    D_x_.append(c)
...
#First order FD
u_t = np.gradient(u_pred, dt, edge_order=1, axis=0)
u_x = np.gradient(u_pred, dx, edge_order=1, axis=1)
u_xx = np.gradient(u_x, dx, edge_order=1, axis=1)
D_x = np.gradient(D[idx], dx, edge_order=1, axis=0)
c =params[idx, 1]
u_residual= u_t - D[idx]*u_xx - u_pred*D_x + c*u_x 
...
# %%
alpha_diff = ((D[idx]*dt)/dx**2)
alpha_conv = c*dt/dx
dD_dx = D_x
u = u_actual[0]

this = -u[1:-1] + (u[1:-1]*(1 - 2*alpha_diff[1:-1]) 
