...
configuration = {"Case": 'Wave',
                 ...
                 "Variables":1, 
                 "Loss Function": 'LP',
                 "UQ": 'Deterministic', #Deterministic, Dropout, Bayesian
                 }
...
with Run(mode='online') as run:
    ...
    if configuration['UQ'] == 'Bayesian':
        from Training.Utils import elbo
        loss_func = elbo
    ...
    pcm = ax.imshow(u_field[0, :, :, 0], cmap=matplotlib.cm.coolwarm, extent=[-1.0, 1.0, -1.0, 1.0], vmin=v_min_1, vmax=v_max_1)
    ...
    pcm = ax.imshow(u_field[0, :, :, int(T_out/ 2)], cmap=matplotlib.cm.coolwarm, extent=[-1.0, 1.0, -1.0, 1.0], vmin=v_min_2,
                    vmax=v_max_2)
    ...
    pcm = ax.imshow(u_field[0, :, :, -1], cmap=matplotlib.cm.coolwarm, extent=[-1.0, 1.0, -1.0, 1.0], vmin=v_min_3, vmax=v_max_3)
