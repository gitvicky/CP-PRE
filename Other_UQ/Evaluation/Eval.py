...
config = config_loc + 'Wave_FNO.yaml'
...
coverage = []
for ii in range(10):
    vars =  vars[np.random.choice(len(vars), size=ntrain, replace=False)]  
...
cov_mean, cov_std = np.mean(coverage), np.std(coverage)

print(f'L2 Mean: {l2_mean}, L2 STD: {l2_std}')
print(f'Cov. Mean: {cov_mean}, Cov. STD: {cov_std}')
