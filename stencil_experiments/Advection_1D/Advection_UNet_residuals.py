...
#Residual Estimations across the spatio-temporal tensor - Utilising the stencil by way of convolutions using pytorch 

u_tensor =torch.tensor(u_pred, dtype=torch.float32).reshape(1,1,u_pred.shape[0], u_pred.shape[1])
...
# %% 
#Plotting the kymograhps fo the test data, surroagte output, absolute error and the physics residuals. 
...
ax.title.set_text('Physics Residual')
ax.set_xlabel('x')
# ax.set_ylabel('t')
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.1)
